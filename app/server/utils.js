...
exports.isAuthorized = function(req, action_techincal_title) {
    return new Promise(function(resolve, reject) {
        ...
        //     group_id = null;
        //     Group.find({'users_role.user': user._id}, function (err, group) {
        //         if(err)
        //             reject(err);
        //         if(null != group) {
        //             group_id = group[0]._id;
        //         } else if(null != req.body.group_id && isNotEmpty(req.body.group_id)) {
...
exports.getUserByToken = function(req) {
    return new Promise(function(resolve, reject) {
        User.find({'_id': req.login._id}, function(err, user) {
            if (err)
                reject(err);
            resolve(user[0]);
        });
